...
j/k的值由 token_offset决定，并从scatter_tokens（$dst_j$）/expert_weights（$weight_k$）中选取。
...
- (attr) num_tokens_per_rank
    - int
    - if shared_experts need split on num_tokens, this is the num_tokens for current rank.
    说明：如果num_shared_experts_per_rank > 1，那么每个共享专家都会独立处理一个rank上所有的token

    假设一个rank上有N个token，编号为0~N。共享专家处理的token，可以如下表示
    [专家0的token0, 专家0的token1, ..., 专家0的tokenN,
     专家1的token0, 专家1的token1, ..., 专家1的tokenN,
    ...]
