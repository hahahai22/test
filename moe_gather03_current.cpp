...
{
    // 计算每个token需要处理的工作量
    int token_idx = blockIdx.x; // 当前rank的token索引
    int col       = threadIdx.x;
    ...
    {
        int src_idx     = (e * num_tokens_per_rank + token_idx) * hidden_size + col;
        float token_val = static_cast<float>(scatter_tokens[src_idx]);

        // 直接累加，不乘权重
        hidden_states[(num_tokens_offset + token_idx) * hidden_size + col] +=
            static_cast<hip_bfloat16>(token_val);
    }

    // 处理非共享专家部分
    for(int k = 0; k < topk; k++)
    {
        int offset = token_offset[token_idx * topk + k];
        int src_idx =
            (num_shared_experts_per_rank * num_tokens_per_rank + offset) * hidden_size + col;
        float token_val = static_cast<float>(scatter_tokens[src_idx]);
        float weight    = expert_weights[(num_tokens_offset + token_idx) * topk + k];

        hidden_states[(num_tokens_offset + token_idx) * hidden_size + col] +=
            static_cast<hip_bfloat16>(token_val * weight);
    }
}
                ...
                hipStream_t stream)
{
    ...
    dim3 grid_size(num_tokens_per_rank);
...
int main()
{
    const int world_size          = 2; // 总共2个rank
    const int num_tokens_per_rank = 6; // 当前rank负责的token数
    const int topk                = 2; // top-k值，必须为2（对于现在的数据初始化）
    const int current_rank        = 1;
    const int num_tokens_offset =
        num_tokens_per_rank *
        current_rank; // 当前rank的token偏移量，确定共享专家处理范围：[num_tokens_offset,
    ...
    for(int e = 0; e < num_shared_experts_per_rank; e++)
    {
        ...
        {
            int current_token = num_tokens_offset + t;
            for(int h = 0; h < hidden_size; h++)
            {
                int src_idx = (e * num_tokens_per_rank + t) * hidden_size + h;
                expected[current_token * hidden_size + h] +=
                    static_cast<float>(h_scatter_tokens[src_idx]);
                // printf("%d, %f\n", src_idx, static_cast<float>(h_scatter_tokens[src_idx]));
            }
        }
    }
    ...
    for(int t = 0; t < num_tokens_per_rank; t++)
    {
        for(int h = 0; h < hidden_size; h++)
        {
            ...
            for(int k = 0; k < topk; k++)
            {
                int offset   = h_token_offset[t * topk + k];
                float weight = h_expert_weights[(num_tokens_offset + t) * topk + k];
                // printf("%f\n", weight);

                int src_idx =
                    (num_shared_experts_per_rank * num_tokens_per_rank + offset) * hidden_size + h;
                expected[current_token * hidden_size + h] +=
                    static_cast<float>(h_scatter_tokens[src_idx]) * weight;
                // printf("%d, %f, %f, %d, %f\n", src_idx,
                // static_cast<float>(h_scatter_tokens[src_idx]), weight, current_token *
                // hidden_size + h, static_cast<float>(h_scatter_tokens[src_idx]) * weight);
                // printf("%d, %f\n", current_token * hidden_size + h, expected[current_token *
                // hidden_size + h]);
            }
            // expected[current_token * hidden_size + h] += expected[current_token * hidden_size +
            // h]; printf("%d, %f\n", current_token * hidden_size + h, expected[current_token *
            // hidden_size + h]);
        }
    }
